...
if __name__ == '__main__':    
    ...
    abductive = T5ForConditionalGeneration.from_pretrained(hps.model_dir)
    deductive = T5ForConditionalGeneration.from_pretrained(hps.model_dir)
    
    if hps.induction == 'atten':
        logger.info('[Indution]: atten')
        inductive = MultiHeadAttenInduction(hps, abductive.config.d_model, tokenizer)
    ...
    for epoch in range(hps.epochs):
        ...
        dev_ab = evaluation_loop_bart(hps, abductive, inductive, dev_loader, tokenizer, mode='abductive')
        ...
        dev_de = evaluation_loop_bart(hps, deductive, inductive, dev_loader, tokenizer, mode='deductive')
        ...
        if dev_ab[0] + dev_de[0] + (dev_ab[1] + dev_de[1]) / 2 >= best_bleu:
            ...
            test_ab = evaluation_loop_bart(hps, abductive, inductive, test_loader, tokenizer, mode='abductive')
            ...
            test_de = evaluation_loop_bart(hps, deductive, inductive, test_loader, tokenizer, mode='deductive')
            ...
            torch.save({'deductive_dict': deductive.state_dict(),
                        ...
                        './output/model/{}_loop_{}_portion{}.ckpt'.format(hps.model_name, hps.induction, round(hps.portion*100)))
