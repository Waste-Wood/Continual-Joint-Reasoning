...
def evaluation(hps, model: BartForConditionalGeneration, dataloader, tokenizer: BartTokenizer):
...
def tokenization(hps, tokenizer: BartTokenizer, causes, effects):
    inputs, outputs = [causes, effects] if hps.mode == 'deductive' else [effects, causes]

    inputs_t = tokenizer(inputs, padding=True, return_tensors='pt')
    outputs_t = tokenizer(outputs, padding=True, return_tensors='pt')
    outputs_t.input_ids[outputs_t.input_ids==tokenizer.pad_token_id] = -100

    return inputs_t.input_ids, inputs_t.attention_mask, outputs_t.input_ids, outputs_t.attention_mask
