...
import tqdm
from utils.tools_alphaNLG import read_data, evaluation_loop_gpt2
from utils.logger import define_logger
from utils.Dataset import DynamicDataset
from torch.utils.data import DataLoader, RandomSampler
from module.gpt2_loop_aNLG import primal, dual
from module.inductive import MultiHeadAttenInduction, HardKumaInduction, ChainInduction
...
def hyper_parameters():
    ...
    parser.add_argument('--induction', type=str, default='comet', choices=['hidden', 'atten', 'chain'])
...
if __name__ == '__main__':    
    ...
    optimizer = AdamW([{'params': abductive.parameters()}, 
                       ...
                       {'params': inductive.parameters()}], lr=hps.lr)

    train_data = read_data(os.path.join(hps.data_dir, hps.train))
    ...
    for epoch in range(hps.epochs):
        ...
        for batch, _ in zip(train_loader, bar):
            ...
            # primal loop
            loss1, loss2 = primal(hps, abductive, deductive, inductive, batch, tokenizer)
            ...
            # dual loop
            optimizer.zero_grad()
            loss1, loss2 = dual(hps, abductive, deductive, inductive, batch, tokenizer)
            if hps.induction == 'hidden':
                loss3 = inductive.get_loss()
            else:
                loss3 = 0
        ...
        dev_ab = evaluation_loop_gpt2(hps, abductive, inductive, dev_loader, tokenizer, mode='abductive')
        ...
        dev_de = evaluation_loop_gpt2(hps, deductive, inductive, dev_loader, tokenizer, mode='deductive')
        ...
        if dev_ab[0] + dev_de[0] + (dev_ab[1] + dev_de[1]) / 2 >= best_bleu:
            ...
            json.dump(dev_ab[-1], output_ab_fi, indent=1)
            json.dump(dev_de[-1], output_de_fi, indent=1)

            torch.save({'deductive_dict': deductive.state_dict(),
                        'abductive_dict': abductive.state_dict(),
                        'indective_de_dict': inductive.state_dict()}, 
                        './output/model/gpt2_loop_{}_{}.ckpt'.format(hps.induction, 'aNLG'))
