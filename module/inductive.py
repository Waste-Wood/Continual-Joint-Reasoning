...
class Embed(nn.Module):
    def __init__(self, hps, model):
        ...
        self.model = model
        if hps.model_name == 'gpt2':
            self.token_embed = model.state_dict()['transformer.wte.weight']
            self.pos_embed = model.state_dict()['transformer.wpe.weight']
            self.dropout = nn.Dropout(0.1)
        else:
            print('zzz')
    
    def forward(self, input_ids):
        if self.hps.model == 'gpt2':
            pos_ids = torch.arange(0, input_ids.size(1), dtype=torch.long, device=input_ids.device)
            token = self.token_embed[input_ids]
            pos = self.pos_embed[pos_ids]
            return self.dropout(token + pos)
        else:
            print('zzz')
...
class MultiHeadAttenInduction(nn.Module):
    ...
    def forward(self, batch_word, batch_knowledge, model, mode):
        ...
        output_embed = get_embedding(self.hps, model, outputs_t[0])
        ...
        model_input_mask = torch.cat([inputs_t[1], outputs_t[1]], 1)
        ...
        modle_input_embeds = torch.cat([atten_add_norm, output_embed], 1)
        
        labels_1 = torch.zeros_like(inputs_t[0], dtype=torch.long).cuda(self.hps.gpu[0]) - 100
        labels_2 = torch.where(outputs_t[0] == self.tokenizer.pad_token_id, -100, outputs_t[0])
        model_input_labels = torch.cat([labels_1, labels_2], 1)

        return modle_input_embeds, model_input_mask, model_input_labels, atten_add_norm, inputs_t[1]
...
class HardKumaInduction(nn.Module):
    ...
    def forward(self, batch_word, batch_knowlwdge, model, mode):
        cause, effect = batch_word
        if mode == 'abductive':
            inputs, outputs = effect, cause
        else:
        ...
        inputs_t = list(self.tokenizer(inputs, padding=True, return_tensors='pt').values())
        ...
        if self.hps.use_gpu:
            kg_input_ids = kg_input_ids.cuda(self.hps.gpu[0])
            kg_atten_mask = kg_atten_mask.cuda(self.hps.gpu[0])
            inputs_t = [term.cuda(self.hps.gpu[0]) for term in inputs_t]
        ...
        modle_input_embeds = torch.cat([attented_word_add_norm, output_embed], 1)
        model_input_mask = torch.cat([inputs_t[1], outputs_t[1]], 1)
        labels_1 = torch.zeros_like(inputs_t[0]) - 100
        labels_2 = torch.where(outputs_t[0] == self.tokenizer.pad_token_id, -100, outputs_t[0])
        labels = torch.cat([labels_1, labels_2], 1)


        return modle_input_embeds, model_input_mask, labels, attented_word_add_norm, inputs_t[1]
...
class ChainInduction(nn.Module):
    ...
    def forward(self, batch_word, batch_knowledge, model, mode):
        ...
        input_embed = get_embedding(self.hps, model, inputs_t[0])
        output_embed = get_embedding(self.hps, model, outputs_t[0])
        kg_embed = [get_embedding(self.hps, model, torch.LongTensor(input_ids).unsqueeze(0)).squeeze(0) for input_ids in kg_input_ids]
        ...
        modle_input_embeds = torch.cat([linear_add_norm, output_embed], 1)
        model_input_mask = torch.cat([inputs_t[1], outputs_t[1]], 1).cuda(self.hps.gpu[0])
        labels_1 = torch.LongTensor(torch.zeros_like(inputs_t[0])).cuda(self.hps.gpu[0]) - 100
        labels_2 = torch.where(outputs_t[0] == self.tokenizer.pad_token_id, -100, outputs_t[0]).cuda(self.hps.gpu[0])
        labels = torch.cat([labels_1, labels_2], 1)

        return modle_input_embeds, model_input_mask, labels, linear_add_norm, inputs_t[1].cuda(self.hps.gpu[0])


class PromptInduction(nn.Module):
    def __init__(self, hps, _, tokenizer):
        super(PromptInduction, self).__init__()
        self.prompt_model = GPT2LMHeadModel.from_pretrained(hps.model_dir).cuda(hps.gpu[0])
        self.prompt_model.requires_grad_ = False
        self.tokenizer = GPT2Tokenizer.from_pretrained(hps.model_dir)
        self.tokenizer.pad_token = self.tokenizer.unk_token
        self.hps = hps
        self.tokenizer = tokenizer
    
    def forward(self, batch_word, _, model, mode):
        cause, effect = batch_word

        if mode == 'abductive':
            effect = [e[:-1] + ' might be caused by ' for e in effect]
            for_gen = self.tokenizer(effect, padding=True, return_tensors='pt')
            res = self.tokenizer(cause, padding=True, return_tensors='pt')
            if self.hps.use_gpu:
                for_gen = [for_gen[term].cuda(self.hps.gpu[0]) for term in for_gen]
                res = [res[term].cuda(self.hps.gpu[0]) for term in res]

        else:
            cause = [c[:-1] + ' might lead to ' for c in cause]
            for_gen = self.tokenizer(cause, padding=True, return_tensors='pt')
            res = self.tokenizer(effect, padding=True, return_tensors='pt')
            if self.hps.use_gpu:
                for_gen = [for_gen[term].cuda(self.hps.gpu[0]) for term in for_gen]
                res = [res[term].cuda(self.hps.gpu[0]) for term in res]

        knowledge_outputs = self.prompt_model.generate(input_ids=for_gen[0],
                                                       ...
                                                       max_length=for_gen[0].shape[-1]+self.hps.max_len,
                                                       pad_token_id=self.tokenizer.eos_token_id)
        
        hidden_states = knowledge_outputs.hidden_states
        words = [state[-1][:, -1, :] for state in hidden_states]
        knowledge_embed = torch.stack(words, 1) # B x L x H
        kg_mask = torch.LongTensor([[1]*self.hps.max_len]*len(cause)).to(knowledge_embed.device) # B x L

        input_embed = get_embedding(self.hps, model, for_gen[0])
        output_embed = get_embedding(self.hps, model, res[0])

        labels_1 = torch.LongTensor([[-100]*(kg_mask.size(1)+for_gen[1].size(1))] * kg_mask.size(0)).to(input_embed.device)
        labels_2 = torch.where(res[0] == self.tokenizer.pad_token_id, -100, res[0])
        labels = torch.cat([labels_1, labels_2], 1)

        model_input_embed = torch.cat([input_embed, knowledge_embed, output_embed], 1)
        model_input_mask = torch.cat([for_gen[1], kg_mask, res[1]], 1)

        return model_input_embed, model_input_mask, labels, torch.cat([input_embed, knowledge_embed], 1), torch.cat([for_gen[1], kg_mask], 1)
...
class CometInduction(nn.Module):
    def __init__(self, hps, hidden_dim, tokenizer):
        super(CometInduction, self).__init__()
        self.hps = hps
        self.comet = BartForConditionalGeneration.from_pretrained(hps.comet_dir)
        self.tokenizer = BartTokenizer.from_pretrained(hps.comet_dir)
        self.reason_tokenier = tokenizer
        self.attention = MultiHeadAttenInduction(hps, hidden_dim, tokenizer)
    
    def forward(self, batch, _, model, mode):
        premise, hypothesis = batch if mode == 'deductive' else batch[::-1]
        
        queries = []
        if mode == 'deductive':
            rels = ["xAttr", "HasSubEvent", "IsBefore", "Causes", "xEffect", "xReact", "xWant"]
        else:
            rels = ["xAttr", "IsAfter", "HasSubEvent", "xReason", "xNeed", "xIntent"]
        
        for p in premise:
            queries += ['{} {} [GEN]'.format(p, r) for r in rels]

        inputs = list(self.tokenizer(queries, padding=True, return_tensors='pt').values())
        if self.hps.use_gpu:
            inputs = [term.cuda(self.hps.gpu[0]) for term in inputs]
        
        with torch.no_grad():
            knowledge_ids = self.comet.generate(
                input_ids = inputs[0],
            ...
            knowledge_text = self.tokenizer.batch_decode(knowledge_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)
            knowledge_text = [process_text(t) for t in knowledge_text]

        knowledge = []
        for i in range(len(batch[0])):
            instance_knowledge = []
            kg = knowledge_text[2*i*len(rels): 2*(i+1)*len(rels)]
            for j in range(len(rels)):
                dimension = kg[j*2: (j+1)*2]
                index = argmax(dimension)
                if dimension[index] == 'none':
                    continue
                instance_knowledge.append(dimension[index])
            instance_knowledge = list(set(instance_knowledge))
            knowledge.append(', '.join(instance_knowledge))
        
        return self.attention([premise, hypothesis], knowledge, model, mode)
        # premise = ['{} {}'.format(k, p) for p, k in zip(premise, knowledge)]

        # prem_t = list(self.tokenizer(premise, padding=True, return_tensors='pt').values())
        # hypo_t = list(self.tokenizer(hypothesis, padding=True, return_tensors='pt').values())

        # if self.hps.use_gpu:
        #     prem_t[1] = prem_t[1].cuda(self.hps.gpu[0])
        #     hypo_t[1] = hypo_t[1].cuda(self.hps.gpu[0])

        # prem_embed = get_embedding(self.hps, model, prem_t[0])
        # hypo_embed = get_embedding(self.hps, model, hypo_t[0])

        # model_input_embed = torch.cat([prem_embed, hypo_embed], 1)
        # model_input_mask = torch.cat([prem_t[1], hypo_t[1]], 1)
        # labels1 = torch.zeros_like(prem_t[0]).cuda(self.hps.gpu[0]) - 100
        # labels2 = torch.where(hypo_t[0] == self.reason_tokenier.pad_token_id, -100, hypo_t[0]).cuda(self.hps.gpu[0])
        # labels = torch.cat([labels1, labels2], 1)

        # return model_input_embed, model_input_mask, labels, prem_embed, prem_t[1]



class RuleInductionMoKGE():
...
class RuleInduction():
    def __init__(self, hps):
        ...
        indexes = json.load(open('/home/kxiong/work/joint_reasoning/data/conceptnet_old/indexes.json', 'r'))
